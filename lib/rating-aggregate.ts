import { calculateRating, RATING_PRIOR_STRENGTH } from "./rating";
import type {
 ...
 * accepted but never trusted: both are derived again from the frozen prior and
 * valid ballot totals.
 */
...
export function parseRatingAggregate(
  ...
  if (priorStrength !== RATING_PRIOR_STRENGTH) {
    throw new Error(
      `Rating aggregate ${statementId} uses unsupported prior strength ${priorStrength}.`
    );
  }
  ...
  const modelVersion = safeInteger(row.model_version, "model version");
  if (modelVersion !== calculated.modelVersion) {
    throw new Error(
      `Rating aggregate ${statementId} uses unsupported model version ${modelVersion}.`
    );
  }
  const updatedAt =
