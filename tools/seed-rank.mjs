#!/usr/bin/env node
/**
 * seed-rank — validate the seed corpus and retain a reproducible compatibility
 * artifact for the retired editorial cold-start design.
 *
 ...
 *
 * WHY THIS REMAINS. Corpus-import verification still records the generated
 * artifact, and the old rubric order is useful when auditing historical design
 * decisions. The live application never reads it as a public score. Public
 * performance is the equal-weight mean of valid database ballots.
 *
...
function pct0(n, d) { return `${(d ? (n / d) * 100 : 0).toFixed(0)}%`; }
...
console.log(`\nLEGACY TIER SPREAD — compatibility artifact only`);
...
console.log(`\nLEGACY EDITORIAL ORDER — not a public ranking`);
...
if (process.argv.includes("--write")) {
  const out = {
    $comment: "GENERATED by tools/seed-rank.mjs — do not edit. This retired editorial order is retained only as a corpus-import compatibility artifact. The public site ranks only equal-weight validated ballot aggregates.",
