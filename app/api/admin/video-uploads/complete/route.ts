...
// The first check validates provider metadata before requesting the async
// H.264/AAC derivative. A later retry verifies the signed derivative response.
export const maxDuration = 60;

type UploadCompleteResponse =
  | { ok: true; result: Awaited<ReturnType<typeof completeCloudinaryUpload>> }
...
export async function POST(request: Request) {
  try {
    ...
    console.error("Failed to complete Cloudinary video upload", error);
